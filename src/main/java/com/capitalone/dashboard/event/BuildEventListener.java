    ...
    private void processBuild(Build build) {
        ...
        for (Dashboard teamDashboard : teamDashboardsReferencingBuild) {
            ...
            for(CollectorItem productCI : productCIs) {
                ...
                for (Map.Entry<String, PipelineCommit> entry : commitStageCommits.entrySet()) {
                    PipelineCommit commit = entry.getValue();
                    if ((commit.getScmCommitTimestamp() < build.getStartTime()) && !buildStageCommits.containsKey(entry.getKey()) && isMoveCommitToBuild(build, commit, commitRepository)) {
                        pipeline.addCommit(PipelineStage.BUILD.getName(), commit);
                    }
                }
                pipelineRepository.save(pipeline);
